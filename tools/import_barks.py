...
This script processes songs from .txt files in song_index directory and exports barks
using BoomyExporter.exe for each song.

Usage:
    python import_barks.py <path_to_boomy_exporter> <input_folder> <game_name>
...
def run_boomy_export(exporter_path, input_folder, song_name, game_name, output_folder):
    """
    Run BoomyExporter.exe to export barks for a specific song.
    ...
    print(f"Exporting barks for '{song_name}'...")
...
def copy_barks(tmp_folder, barks_folder):
    """
    Copy bark files from temporary folder to barks directory.
    Overrides existing files.
    
    Args:
        tmp_folder: Temporary folder containing exported files
        barks_folder: Target barks directory
    """
    barks_source_folder = os.path.join(tmp_folder, "barks")
    
    if not os.path.exists(barks_source_folder):
        print(f"Warning: Barks folder '{barks_source_folder}' does not exist")
        return False
    
    if not os.listdir(barks_source_folder):
        print(f"Warning: Barks folder '{barks_source_folder}' is empty")
        return False
    
    try:
        # Create barks folder if it doesn't exist
        os.makedirs(barks_folder, exist_ok=True)
        
        # Copy all files from barks to target folder (override existing)
        for item in os.listdir(barks_source_folder):
            src_path = os.path.join(barks_source_folder, item)
            dst_path = os.path.join(barks_folder, item)
        ...
        print(f"Error copying bark files: {e}")
        return False
...
def main():
    """Main function to process all songs for barks export."""
    if len(sys.argv) != 4:
        print("Usage: python import_barks.py <path_to_boomy_exporter> <input_folder> <game_name>")
        print("\nExample:")
        print("    python import_barks.py BoomyExporter.exe D:\\songs dc3")
        sys.exit(1)
    ...
    # Get current working directory for barks output
    cwd = os.getcwd()
    barks_folder = os.path.join(cwd, "barks")
    
    print(f"BoomyExporter: {exporter_path}")
    print(f"Input folder: {input_folder}")
    print(f"Game name: {game_name}")
    print(f"Barks will be copied to: {barks_folder}")
    print("-" * 50)
    ...
    for i, song_name in enumerate(songs, 1):
        ...
        with tempfile.TemporaryDirectory() as tmp_folder:
            ...
            # Export barks using BoomyExporter
            if run_boomy_export(exporter_path, input_folder, song_name, game_name, tmp_folder):
                # Copy bark files to barks directory (override existing)
                if copy_barks(tmp_folder, barks_folder):
                    successful_exports += 1
                    print(f"Successfully processed '{song_name}'")
                else:
                    print(f"Failed to copy barks for '{song_name}'")
            else:
                print(f"Failed to export '{song_name}'")
    
    # Summary
    print("\n" + "=" * 50)
    print(f"Barks import completed!")
    print(f"Successfully processed: {successful_exports}/{total_songs} songs")
    print(f"Barks directory: {barks_folder}")
